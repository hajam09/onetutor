...
class TestAccountViewsRegister(TestCase):
	...
	def test_login_GET(self):
		...
		self.assertTemplateUsed(response, ValueSet.ACCOUNTS_REGISTRATION_TEMPLATE)
	...
	def test_register_account_exists(self):
		...
		self.assertEquals(response.context["email"], ValueSet.USER_1_EMAIL)
	...
	def test_register_password_not_matching(self):
		...
		self.assertIn("email", response.context)
		self.assertEquals(response.context["email"], ValueSet.USER_2_EMAIL)
		self.assertIn("firstname", response.context)
	...
	def test_register_create_account(self):
		...
		self.assertTrue(User.objects.filter(username=ValueSet.USER_2_EMAIL).exists())